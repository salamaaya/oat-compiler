...
let insn_flow ((u,i):uid * insn) (d:fact) : fact =
  ...
  | Gep (_, Id u', _)
  | Load (Ptr (Ptr _), Id u')
  | Store (Ptr _, Id u', _) -> UidM.add u' SymPtr.MayAlias new_d
  | _ -> new_d
  ...
  struct
    ...
    *)
    let combine (ds:fact list) : fact =
      let merge_helper (u:uid) (s1:SymPtr.t option) (s2:SymPtr.t option) : SymPtr.t option = 
        match s1, s2 with 
        | None, None -> None 
        | Some _, None -> s1 
        | None, Some _ -> s2
        | Some SymPtr.MayAlias, Some _ 
        | Some _, Some MayAlias -> Some SymPtr.MayAlias
        | Some SymPtr.Unique, Some _ 
        | Some _, Some SymPtr.Unique -> Some SymPtr.Unique
        | _ -> None
      in 
