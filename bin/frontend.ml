...
let oat_alloc_struct (ct:TypeCtxt.t) (id:Ast.id) : Ll.ty * operand * stream =
  ...
  let size = Const (TypeCtxt.lookup id ct |> List.length |> Int64.of_int) in 
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
    let t, o, s = cmp_exp tc c e in
    let arr_ty = begin match t with 
    | Ptr (Struct [_; Array(_, arr_ty)]) -> arr_ty
    | _ -> failwith "invalid array"
    end in 
    let length = gensym "length" in
    let ptr = gensym "ptr" in 
    I64, Id length, s >@ [I(ptr, Gep(t, o, [Const 0L; Const 0L]))] 
                      >@ [I(length, Load (Ptr arr_ty, Id ptr))]
  ...
  *)
  | Ast.NewArrInit (elt_ty, e1, id, e2) ->    
    ...
    let arr_ty, arr_op, alloc_code = oat_alloc_array tc elt_ty size_op in
    let arr_ptr = gensym "ptr" in
    let lhs_id = no_loc @@ Id id in
    let guard = Bop (Lt, no_loc (Lhs (lhs_id)), e1) in
    let after = Assn (lhs_id, no_loc @@ Bop (Add, no_loc (Lhs (lhs_id)), no_loc @@ CInt 1L)) in 
    let stmt = For ([(id, no_loc @@ CInt 0L)], Some (no_loc guard), Some (no_loc after), [no_loc @@ Assn (no_loc @@ Index(no_loc @@ Lhs (no_loc @@ Id arr_ptr), no_loc @@ Lhs lhs_id), e2)]) in
    let c' = Ctxt.add c arr_ptr (Ptr arr_ty, Id arr_ptr) in
    let _, s = cmp_stmt tc c' arr_ty (no_loc stmt) in
    arr_ty, arr_op, size_code >@ alloc_code >@ 
      [I (arr_ptr, Alloca(arr_ty))] >@
      [I ("", Store (arr_ty, arr_op, Id arr_ptr))] >@ s
  ...
  | Ast.CStruct (id, l) ->
    ...
    let add_elt s (i, elt) =
      let elt_ty, elt_op, elt_code = cmp_exp tc c elt in
      let ind = gensym "ind" in 
      s >@ elt_code >@ lift
        [ ind, Gep(struct_ty, struct_op, [Const 0L; i64_op_of_int (TypeCtxt.index_of_field id i tc) ])
        ; gensym "store",  Store(elt_ty, elt_op, Id ind) ] 
    in
    let ind_code = List.(fold_left add_elt [] l) in
    struct_ty, struct_op, alloc_code >@ ind_code

  | Ast.Lhs l ->
     let ans_ty, assignable, ptr_op, code = cmp_lhs tc c l in
     if assignable then
       let ans_id = gensym "lhs" in
...
(* compiles a lhs:
   returns
   - the LLVM type of the result (_without_ the Ptr if it assignable)
   - a flag indicating whether the lhs is assignable
     (and therefore whether it should be dereferenced as an expression)
   - the operand holding the pointer 
   - the instruction stream needed to compute the pointer
 *)
and cmp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (l:lhs node) : Ll.ty * bool * Ll.operand * stream =
  match l.elt with
  | Ast.Id x ->
    let pt, op = Ctxt.lookup x c in
    begin match pt with
    | Ptr (Fun _) -> pt, false, op, []
    | Ptr t ->   t, true, op, []
  ...
  (* STRUCT TASK: Complete this code that emits LL code to compute the
     ...
     Ast.Lhs case of the cmp_exp function (above).
  ...
  *)
  | Ast.Proj (e, i) ->
    ...
    let struct_name = match struct_ty with 
      | Ptr (Namedt id) -> id 
      | _ -> failwith "invalid struct" in
    let ans_ty, ind = TypeCtxt.lookup_field_name struct_name i tc in
    let ptr_id, tmp_id, call_id= gensym "index_ptr", gensym "tmp", gensym "call" in
    cmp_ty tc ans_ty, true, (Id ptr_id),
    struct_code >@ lift
      [ptr_id, Gep(struct_ty, struct_op, [Const 0L; Const ind]) ]
  ...
  *)
  | Ast.Index (e, i) ->
    ...
    let ptr_id, tmp_id, call_id, bitcast = gensym "index_ptr", gensym "tmp", gensym "call", gensym "bitcast" in
    ans_ty, true, (Id ptr_id),
    arr_code >@ ind_code >@ lift
      [ bitcast, Bitcast(arr_ty, arr_op, Ptr I64)
      ; "", Call(Void, Gid "oat_assert_array_length", [Ptr I64, Id bitcast; I64, ind_op])
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  | Ast.Assn (path ,e) ->
     let ll_ty, _, pop, path_code = cmp_lhs tc c path in
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    ...
    let cast, check_null, lt, le, lm, alloca = gensym "cast", gensym "is_null", gensym "then", gensym "else", gensym "merge", gensym "alloca" in
    let ref_ty = cmp_ty tc (TRef typ) in 
    let c' = Ctxt.add c id (Ptr ref_ty, Id alloca) in
    let _, then_code = cmp_block tc c' rt notnull in
    let _, else_code = cmp_block tc c rt null in
    c, guard_code 
      >:: I(cast, Bitcast(guard_ty, guard_op, ref_ty))
      >:: I(check_null, Icmp (Ne, ref_ty, Id cast, Null))
      >:: T(Cbr (Id check_null, lt, le))
      >:: L lt >:: E(alloca, Alloca ref_ty) >:: I("", Store (ref_ty, Id cast, Id alloca)) >@ then_code >:: T(Br lm) 
      >:: L le >@ else_code >:: T(Br lm) 
      >:: L lm
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    let elts, gs = List.fold_right
        (fun cst (elts, gs) ->
           let gd, gs' = cmp_gexp c tc (snd cst) in
           gd::elts, gs' @ gs) cs ([], [])
    in
    let ll_u = List.map fst elts in 
